...
The VM's startup script ([`startup.sh`](startup.sh)) automatically runs on boot to configure package repositories, Go, tools, system-wide environment PATH, and Antigravity. It uses a sentinel file check (`/var/log/startup_script_done`) to ensure heavy setup runs only on the first boot.
